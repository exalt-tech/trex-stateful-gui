...
import com.exalttech.trex.stateful.models.rxcheck.HistogramItem;
 ...
 * @author BassamJ
 */
public class RxHistogramChartController implements Initializable {

    private static final Logger LOG = Logger.getLogger(RxHistogramChartController.class.getName());

    private static final int MAX_DATA_POINTS = 20;
    @FXML
    private AreaChart rxHistogramChart;
    @FXML
    ...
    private void initializeChart() {
        ...
        rxHistogramChart.setCreateSymbols(false);
        rxHistogramChart.setLegendVisible(false);
        rxHistogramChart.setAnimated(false);
        rxHistogramChart.setHorizontalGridLinesVisible(true);

        histogramSeries.setName("latency history");
        rxHistogramChart.getData().add(histogramSeries);
        initializeService();
    }
    ...
    public void updateChartData(List<HistogramItem> chartItems) {
        histogramSeries.getData().removeAll(histogramSeries.getData());
                ...
                try {
                    if (AsyncResponse.getInstance().getRxCheckObject() != null) {
                        if (AsyncResponse.getInstance().isServerCrashed()) {
                            //reset Data in histogram chart
                            histogramSeries.getData().clear();
                        } else {
                            updateChartData(AsyncResponse.getInstance().getRxCheckObject().getData().getLatencyHist().getHistogram());
                        }
                    }
                } catch (Exception ex) {
